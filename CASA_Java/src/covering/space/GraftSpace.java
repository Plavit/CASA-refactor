...
public class GraftSpace extends CoveringArraySpace implements Listener<SearchFinish> {

    private static final int MAXIMUM_SUBROW_CHANGE_FAILED_ATTEMPTS = 32;

    protected Vector<Array<Integer>> resurrectionBuffer = new Vector<>();

    public GraftSpace(int strength, Options options) {
    ...
    public Set<CoveringArray> getChildren(CoveringArray state, int count) {
        ...
        for (int i = count; i > 0; ++attempts) {
            ...
            int combinationIndex = rand.nextInt() % noncoverage.size();
            while (combinationIndex-- > 0) {
                //TODO
                //++combinationIterator;
            }
            //TODO
            Array<Integer> combination = TODO;//*combinationIterator;
            Array<Integer> columns = new Array<>(strength);
            InputKnown known = new InputKnown();
            if (attempts < MAXIMUM_SUBROW_CHANGE_FAILED_ATTEMPTS) {
                for (int j = strength; j > 0; j--) {
                    columns.set(j, options.getOption(combination.get(j)));
                }
                for (int j = options.getSize(), k = strength; j > 0; j--) {
                    if ((k > 0) && columns.get(k - 1) == j) {
                        int symbol = combination.get(--k);
                        known.append(new InputTerm(false, symbol));
                    } else {
                        assert(!(k > 0) || columns.get(k - 1) < j);
                        int symbol = state.getArrayValue(row, j);
                        known.append(new InputTerm(false, symbol));
                    }
                }
                ...
                for (int k = strength; k > 0; k--) {
                    known.append(new InputTerm(false, combination.get(k)));
                }
                CoveringArray child = state;
                //TODO
    ...
    public void clearResurrectionBuffer() {
